...
public class EntrySetTester
{
    ...
    public void containsAllTest()
    {
        ...
        assertFalse(eSet.containsAll(test2));

        assertTrue(eSet.containsAll(test));
     ...
     * un for che tutti gli Entry di eSet(ottenuti con un toArray(). Lo scorro al contrario poiché l'iteratore naviga nel set in ordine
     * inverso (per definizione di Enumeration)) siano uguali agli Entry che scorre l'iteratore test tramite il metodo
     ...
     * rimuova effettivamente l'ultimo Entry ispezionato da test1 controllando che la dimensione del set sia
    ...
    public void removeAllTest()
    {
        ...
        assertTrue(eSet.removeAll(test));
        assertEquals(0, eSet.size());
    ...
    public void retainAllTest()
    {
        ...
        assertEquals(10, eSet.size());
        assertFalse(eSet.retainAll(test));
        assertEquals(10, eSet.size());

        int size = eSet.size();
        assertTrue(eSet.retainAll(test3));
        for (int i = 0; i < eSet.size(); i++)
     ...
     * <br><br>
     * <b>Sommario</b>: Svuoto map, ci aggiungo una mappatura k/v e rigenero eSet. Creo due mapAdapter test1, test2
     * inizialmente vuote, aggiungo ad ognuna una mappatura k/v diversa (quella di test1 è uguale a quella di map)
     * e genero i relativi entrySet() in testSet1 e testSet2. Controllo che l'hashcode di eSet sia uguale all'hashcode di testSet1 e
     ...
     * <b>Sommario</b>: Svuoto map, ci aggiungo una mappatura k/v e rigenero eSet. Creo due mapAdapter test1, test2
     * inizialmente vuote, aggiungo ad ognuna una mappatura k/v diversa (quella di test1 è uguale a quella di map)
     * e genero i relativi entrySet() in testSet1 e testSet2. Verifico che il metodo equals() dia esito positivo se confronto
     * eSet con testSet1 (poiché uguali) e che dia esito negativo se confronto eSet con testSet2 (poiché diversi).
     * Controllo che ritorni false se invoco il metodo con una lista nulla e un oggetto non di tipo EntrySet.
     * <br><br>
     * <b>Design</b>: corretto controllo della uguaglianza fra due EntrySet anche nei casi limite dove la lista passata come
     * argomento sia nulla o non di tipo EntrySet.
     * <br><br>
     * <b>Pre-condizioni</b>: oggetto di tipo HSet correttamente inizializzato con dimensione uguale a zero.
     * <br><br>
     * <b>Post-Condizioni</b>: eSet torna ad essere vuota e di dimensione zero.
     * <br><br>
     * <b>Risultato Atteso</b>: Il metodo ritornerà true se l'oggetto passato come argomento sia un EntrySet uguale a eSet,
     * altrimenti false.
     */
    @Test
