...
public class KeySetTester
{
    ...
    /**
     * Istanzia un nuovo KeySet pieno prima di ogni test.
     * */
    @Before
    ...
    {
        assertThrows(UnsupportedOperationException.class, () -> {
            kSet.addAll((HCollection) getACollection());});
    }
    ...
    public void containsAllTest()
    {
        ...
        assertFalse(kSet.containsAll((HCollection) test2));

        assertTrue(kSet.containsAll((HCollection) test));

        for (int i = 0; i < kSet.size(); i++)
            assertTrue(kSet.contains(test.toArray()[i]));

        assertThrows(NullPointerException.class, () -> {
            kSet.containsAll(null);});
        assertThrows(ClassCastException.class, () -> {
            kSet.containsAll((HCollection) getAVector());});
    }
     ...
     * e controllo che la dimensione di kSet sia effettivamente 10. Verifico che rimuovendo tutte le chaivi
    ...
    public void removeAllTest()
    {
        ...
        assertTrue(kSet.removeAll((HCollection) test));
        assertEquals(0, kSet.size());

        for (int i = 0; i < 10; i++) map.put(i + 20, i);
        kSet = map.keySet();

        assertFalse(kSet.removeAll((HCollection) test2));
        assertEquals(10, kSet.size());

        assertThrows(NullPointerException.class, () -> {
            kSet.removeAll(null);});
        assertThrows(ClassCastException.class, () -> {
            kSet.removeAll((HCollection) getAVector());});
    }
    ...
    public void retainAllTest()
    {
        ...
        assertEquals(10, kSet.size());
        assertFalse(kSet.retainAll((HCollection) test));
        assertEquals(10, kSet.size());

        int size = kSet.size();
        assertTrue(kSet.retainAll((HCollection) test3));
        for (int i = 0; i < kSet.size(); i++)
        ...
        assertThrows(NullPointerException.class, () -> {
            kSet.retainAll(null);});
        assertThrows(ClassCastException.class, () -> {
            kSet.retainAll((HCollection) getAVector());});
    }
     ...
     *<br><br>
     * <b>Sommario</b>: Istanzio un nuovo array test di Object della dimesione di kSet e ci copio all'interno una chiave per
     ...
     * Object; il primo, test,  della dimesione di kSet, il secondo, test1, della dimensione di (kSet + 5) e il terzo,
